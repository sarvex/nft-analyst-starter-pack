...
def get_recent_block(update_log_file, contract_address, web3):
    ...
    if os.path.isfile(update_log_file):
        ...
        print(f"Starting with block {str(most_recent_block)}")
    else:
        ...
        print(
            f"No existing data. Contract {contract_address} appears to have been deployed at block {most_recent_block}"
        )
